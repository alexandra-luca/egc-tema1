...
void Laborator2::Init()
{
...

	{
		...
		meshes["patr"] = new Mesh("generated patr");
		meshes["patr"]->InitFromData(vertices, indices);

		// Create a new mesh from buffer data
		Mesh* patr = CreateMesh("patr", vertices, indices);
	}
}
...
void Laborator2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["cube3"], shaders["VertexNormal"], glm::vec3(1, 0.5f, 1.5f), glm::vec3(0.75f));

	RenderMesh(meshes["tetra"], shaders["VertexColor"], glm::vec3(2, 0.5f, 0), glm::vec3(0.75f));

	RenderMesh(meshes["patr"], shaders["VertexColor"], glm::vec3(0, -1, 0), glm::vec3(0.75f));
